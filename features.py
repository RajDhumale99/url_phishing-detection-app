...
with open("mini_dataset/2.html", encoding="utf-8") as f:
    test = f.read()
...
# has_title
def has_title(soup):
    return 1 if soup.title and soup.title.string and len(soup.title.string.strip()) > 0 else 0
...
def has_password(soup):
    for input in soup.find_all("input"):
        if input.get("type") == "password" or input.get("name") == "password" or input.get("id") == "password":
            return 1
    return 0
...
def has_email_input(soup):
    for input in soup.find_all("input"):
        if input.get("type") == "email" or input.get("name") == "email" or input.get("id") == "email":
            return 1
    return 0
...
def length_of_title(soup):
    if soup.title and soup.title.string:
        return len(soup.title.string.strip())
    else:
        return 0
